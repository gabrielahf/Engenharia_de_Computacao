...
class BST {
    private :
        ...
        int fb(NodeBT <T > * node , const T &e ); 
        NodeBT <T > * root ;
    public :
        BST ();
        ~BST ();
...
template <typename T>
int BST <T>::fb ( NodeBT <T> * node , const T &e) {
    if ( e == node->getInfo() ) {
        int left = 0, right = 0;
        if(node->getRight() != nullptr) right = 1 + node->getRight()->height();
        if(node->getLeft() != nullptr) left = 1 + node->getLeft()->height();
        return right - left;
    }
    
    if ( e < node->getInfo() ) { // Left
        NodeBT <T> * left = node -> getLeft ();
        if ( left == nullptr ) return -900;
        else return fb ( left , e );
    }
    else { // Right
        NodeBT <T> * right = node -> getRight ();
        if ( right == nullptr ) return -900;
        else return fb ( right ,e );
    }
}

template <typename T>
int BST <T>::fb (const T &e) {
    if ( root == nullptr ) return -900;
    return fb ( root, e );
}
